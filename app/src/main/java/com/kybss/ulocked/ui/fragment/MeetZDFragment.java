...
import com.kybss.ulocked.widget.ChooseFloorDialogZD;
...
public class MeetZDFragment extends BaseMeetFragment<Meeting, MeetController.MeetUiCallbacks>
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.mRlScan:
                ...
                break;
            case R.id.mRlLoc:
                new ChooseFloorDialogZD(getActivity(), new ChooseFloorDialogZD.OnConfirmListener() {
                    @Override
                    public void onClick(String curr_building, String curr_floor) {
                        ToastUtil.showToast(curr_building + ";"+curr_floor);
                        getCallbacks().searchFloor("ZDY",curr_floor);
                    }
                }).setDate(floors).show();
                break;
        }
